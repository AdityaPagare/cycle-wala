...
/* Cycle Wala palette instead of the reference's brand-agnostic set. */
const COLORS = ["#4CAF2E", "#3c8a24", "#141414", "#0891A6", "#D97A00", "#8a8a90"];

/* Local placeholder textures — the reference pulled these from an external
   Cloudflare Images account (someone else's hosted photos, unrelated to this
   site); replaced with our own local assets so nothing depends on a
   third-party account or shows unrelated imagery. */
const IMAGES = [
  "/images/placeholder-1.svg",
  "/images/placeholder-2.svg",
  "/images/placeholder-3.svg",
  "/images/hero-bike.svg",
  "/images/placeholder-1.svg",
];
...
export default function TunnelIntro({ text = "CYCLE WALA" }: { text?: string }) {
  ...
  useEffect(() => {
    ...
    const buildMask = () => {
      ...
      if (!ctx) return;
      ctx.scale(dpr, dpr);
      /* the Cycle Wala brand font (rounded, closer to the real logo's
         letterforms than Inter) — --font-brand is set on <html> by
         next/font and inherits down, so any descendant can read it */
      const family =
